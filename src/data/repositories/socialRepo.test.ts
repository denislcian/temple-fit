...
describe('socialRepo (modo local)', () => {
  ...
  it('solo deja eliminar publicaciones propias (las demo están protegidas)', async () => {
    ...
    const own = await socialRepo.publish({ author: 'Dani', text: 'Borrar luego', kind: 'texto' });
    await expect(socialRepo.removeOwnPost('demo-marta', 'Dani')).rejects.toThrow(/propias/);
    await expect(socialRepo.removeOwnPost(own.id, 'Otra persona')).rejects.toThrow(/propias/);
    await socialRepo.removeOwnPost(own.id, 'Dani');
    const feed = await socialRepo.getFeed();
